...
const sendWhatsApp = () => {
    ...
    // Parse localStartTime (format: "YYYY-MM-DDTHH:mm") sin asumir timezone
    // para evitar desfases al crear con new Date()
    const [datePart, timePart] = localStartTime.value.split('T')
    const [year, month, day] = datePart.split('-').map(Number)
    const [hour, minute] = (timePart || '00:00').split(':').map(Number)
    // Crear fecha en hora local explícitamente
    ...
    if (template && template.trim() !== '') {
        ...
        // Default cuando no hay plantilla configurada
        const stylistName = props.stylists.find(s => s.id === appointment.value.stylist_id)?.full_name || ''
        const conEstilista = stylistName ? ` con ${stylistName}` : ''
        message = `Hola ${c.full_name}, te recordamos tu cita el ${date} a las ${time}${conEstilista}.`
    }
